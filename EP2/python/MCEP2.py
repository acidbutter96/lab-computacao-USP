...
def hit_and_miss(f,a=0,b=1,N=1000,graph=False,log=False):
	...
	#x,y = xy[0],xy[1]
	fig, ax = plt.subplots()
	ax.scatter(X,Y,color='#008080',alpha=.5,label='Pontos')
	#ax.scatter(x,y,'#008080',label='Erro',marker='+')
	...
	if graph:
		#ax.xlabel('x')
		#ax.ylabel('f(x)')
		#ax.title('Acertos e erros')
		fig.show()	
		return [X,Y,I,xy,mean,var,err,Ie,fig]
	else:
